...
<main class="max-w-7xl mx-auto px-4 py-12 space-y-10">
    ...
    <section class="space-y-2 text-center">
        ...
        <p class="text-sm text-gray-500">Ikuti kegiatan terbaru, prestasi mahasiswa, dan wawasan teknologi dari Lab MMT</p>
    </section>
    ...
    <section class="bg-white border border-gray-200 rounded-xl shadow-sm p-4">
        <form action="" method="GET" class="flex flex-col md:flex-row gap-4 items-center">
            ...
            <div class="relative flex-1 w-full">
                ...
                <input type="text" name="search" value="<?php echo h($search); ?>" placeholder="Cari judul berita..." class="w-full pl-11 pr-4 py-3 text-sm rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-orange-400">
            </div>

            <div class="relative w-full md:w-64">
                <span class="absolute inset-y-0 left-0 flex items-center pl-4 text-gray-400 text-xs font-semibold">Kategori:</span>
                <select name="category" onchange="this.form.submit()" class="w-full pl-20 pr-10 py-3 text-sm font-semibold bg-white text-gray-700 border border-gray-200 rounded-lg appearance-none focus:outline-none focus:ring-2 focus:ring-orange-400">
    ...
    <section class="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden p-6 md:p-8 hover:shadow-md transition-shadow duration-300">
        <div class="flex flex-col lg:flex-row gap-8 items-center">
            <div class="w-full lg:w-1/2 h-64 lg:h-80 rounded-xl overflow-hidden bg-gray-100 relative group">
                <img src="<?php echo !empty($featuredNews['gambar_berita']) ? h($featuredNews['gambar_berita']) : 'https://placehold.co/800x500?text=No+Image'; ?>" class="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-700">
            </div>
            <div class="w-full lg:w-1/2 flex flex-col items-start text-left space-y-3">
                <div class="text-xs font-bold text-orange-500 uppercase tracking-wider flex items-center gap-2">
                    ...
                    Terbaru &bull; <?php echo !empty($featuredNews['created_at']) ? h(date('d M Y', strtotime($featuredNews['created_at']))) : ''; ?>
                </div>
                ...
                <a href="#" class="inline-flex items-center justify-center bg-orange-500 hover:bg-orange-600 text-white text-sm font-semibold py-3 px-6 rounded-lg transition-all shadow-md shadow-orange-200 transform hover:-translate-y-0.5">
                    Lihat Selengkapnya
        ...
        <div class="max-w-screen-xl mx-auto px-0">
                    ...
                    <?php foreach ($newsList as $news): ?>
                        <article class="flex flex-col h-full bg-white border border-gray-100 rounded-2xl p-5 shadow-sm hover:shadow-lg transition-all duration-300 group">
                            <div class="h-48 rounded-xl overflow-hidden bg-gray-100 mb-4 relative">
                                <img src="<?php echo !empty($news['gambar_berita']) ? h($news['gambar_berita']) : 'https://placehold.co/600x400?text=No+Image'; ?>" class="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-500">
                            ...
                            <div class="flex flex-col flex-grow">
                                ...
                                <a href="#" class="inline-flex items-center justify-center gap-2 w-full bg-orange-500 hover:bg-orange-600 text-white text-xs font-semibold py-3 px-4 rounded-lg transition-all mt-auto shadow-sm hover:shadow-md">
                                    Lihat Selengkapnya
            ...
            <div class="mt-10 flex justify-center">
                <div class="inline-flex items-center gap-1 bg-white border border-gray-200 rounded-xl px-3 py-2 shadow-sm text-sm font-semibold">
                    <a href="<?php echo $link($prev); ?>" class="px-3 py-1 rounded-lg <?php echo $page <= 1 ? 'text-gray-300 cursor-not-allowed' : 'text-orange-600 hover:bg-orange-50'; ?>">Sebelumnya</a>
                    <?php for ($i = $startPage; $i <= $endPage; $i++): ?>
                        <a href="<?php echo $link($i); ?>" aria-current="<?php echo $i === $page ? 'page' : 'false'; ?>" class="px-3 py-1 rounded-lg <?php echo $i === $page ? 'bg-orange-500 text-white shadow-sm' : 'text-gray-700 hover:bg-orange-50'; ?>">
                            <?php echo $i; ?>
                        </a>
                    <?php endfor; ?>
                    <a href="<?php echo $link($next); ?>" class="px-3 py-1 rounded-lg <?php echo $page >= $totalPages ? 'text-gray-300 cursor-not-allowed' : 'text-orange-600 hover:bg-orange-50'; ?>">Berikutnya</a>
                </div>
            </div>
        </div>
    </section>
