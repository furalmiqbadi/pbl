...
<div class="w-64 h-screen bg-[#0f172a] text-white fixed top-0 left-0 flex flex-col z-50 shadow-2xl">
    ...
    <nav class="flex-1 py-6 space-y-2 overflow-y-auto px-3 scrollbar-hide">
        ...
        <a href="dashboard.php?page=dosen" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group <?php echo ($activePage == 'dosen' || $activePage == 'tambah_dosen') ? 'bg-orange-600 text-white shadow-lg' : 'text-gray-400 hover:bg-gray-800'; ?>">
        ...
        <a href="dashboard.php?page=backup" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group <?php echo $activePage == 'backup' ? 'bg-orange-600 text-white shadow-lg' : 'text-gray-400 hover:bg-gray-800'; ?>">
